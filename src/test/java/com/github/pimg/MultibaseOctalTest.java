...
class MultibaseOctalTest {
    ...
    void testStringDecode() {
        ...
        assertEquals("this is a test", new String(result, StandardCharsets.US_ASCII));
    }
    ...
    void testDecodeFullAlphabetString() {
        ...
        byte[] result = Multibase.OCTAL.getDecoder().decode(testString);
        assertEquals("The quick brown fox jumps over the lazy dog.", new String(result, StandardCharsets.US_ASCII));
    }
    ...
    void testEncodeToString() {
        ...
        String result = Multibase.OCTAL.getEncoder().encodeToString(testString.getBytes(StandardCharsets.US_ASCII));
    ...
    void testEncodeFullAlpabetToBytes() {
        ...
        byte[] result = Multibase.OCTAL.getEncoder().encode(testString.getBytes(StandardCharsets.US_ASCII));
        assertArrayEquals("7124 150 145 40 161 165 151 143 153 40 142 162 157 167 156 40 146 157 170 40 152 165 155 160 163 40 157 166 145 162 40 164 150 145 40 154 141 172 171 40 144 157 147 56".getBytes(StandardCharsets.US_ASCII), result);
    }
}
